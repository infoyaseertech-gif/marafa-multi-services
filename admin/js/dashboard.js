let staff = [], projects = [], sales = [], expenses = [];
...
function computeAndRenderDashboard() {
  ...
  const monthMap = {};
  sales.forEach((s) => { const m = (s.sale_date || "").slice(0, 7); if (!m) return; monthMap[m] = monthMap[m] || { sales: 0, expenses: 0 }; monthMap[m].sales += Number(s.amount) || 0; });
  expenses.forEach((e) => { const m = (e.expense_date || "").slice(0, 7); if (!m) return; monthMap[m] = monthMap[m] || { sales: 0, expenses: 0 }; monthMap[m].expenses += Number(e.amount) || 0; });
  ...
  const statusCounts = { Planned: projects.filter((p) => p.status === "Planned").length, Ongoing: ongoing, Completed: completed };
...
(async function init() {
  ...
  renderShell("dashboard", company, profile.full_name || profile.email);
  [staff, projects, sales, expenses] = await Promise.all([getStaff(), getProjects(), getSales(), getExpenses()]);
