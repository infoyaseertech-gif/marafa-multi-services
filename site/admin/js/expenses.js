...
function expenseFormHtml(e) {
  e = e || { expense_date: "", category: EXPENSE_CATEGORIES[0], amount: "", description: "", paid_to: "" };
  return `
    <form id="expenseForm">
      <div class="form-row-2">
        <div class="form-field"><label>Date</label><input id="f_date" type="date" required value="${esc(e.expense_date)}"></div>
      ...
      <div class="form-row-2">
        ...
        <div class="form-field"><label>Paid To</label><input id="f_paidTo" value="${esc(e.paid_to)}"></div>
      </div>
...
function openExpenseModal(existing) {
  ...
  modal.querySelector("#expenseForm").addEventListener("submit", async (e) => {
    e.preventDefault();
    const data = {
      expense_date: modal.querySelector("#f_date").value,
      ...
      paid_to: modal.querySelector("#f_paidTo").value.trim(),
    };
...
function render() {
  const filtered = expenses.filter((e) => (e.category + e.description + e.paid_to).toLowerCase().includes(searchTerm.toLowerCase()));
  ...
  document.querySelectorAll("[data-del]").forEach((btn) => btn.addEventListener("click", async () => {
    if (confirm("Delete this expense record?")) { await deleteExpense(btn.dataset.del); load(); }
  }));
}

async function load() { expenses = await getExpenses(); render(); }

(async function init() {
  const { profile } = await requireAuth();
  const company = await getCompany();
  renderShell("expenses", company, profile.full_name || profile.email);
  await load();
})();
